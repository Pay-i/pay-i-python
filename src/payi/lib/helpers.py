...
def increment_kpi_score(payi: Payi, kpi_name: str, use_case_name: str, use_case_id: str, increment: int = 1) -> bool:
    try:
        ...
        if logger:
            logger.debug(
                f"Failed to increment KPI {kpi_name}, use_case_name {use_case_name}, use_case_id {use_case_id}: {e}"
            )
...
async def increment_kpi_score_async(
    payi: AsyncPayi, kpi_name: str, use_case_name: str, use_case_id: str, increment: int = 1
) -> bool:
    try:
        current_kpi_value = 0
