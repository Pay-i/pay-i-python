...
class PayiInstrumentor:
    ...
    def __init__(
        ...
        prompt_and_response_logger: Optional[Callable[[str, 'dict[str, str]'], None]] = None, # (request id, dict of data to store) -> None
        ):
    ...
    def _instrument_specific(self, instruments: Set[Instruments]):
    ...
    def _ingest_units(
        ...
        if int(ingest_units.get('http_status_code') or 0) < 400:
            ...
            if not units or all(unit.get('input', 0) == 0 and unit.get('output', 0) == 0 for unit in units.values()):
                logging.error("No units to ingest.  For OpenAI streaming calls, make sure you pass stream_options={\"include_usage\": True}")
                return
    ...
    @staticmethod    
    def payi_wrapper(func: Any):
        def _payi_wrapper(o: Any):
...
class ChatStreamWrapper(ObjectProxy): # type: ignore
    def __init__(
        ...
        self._log_prompt_and_response: bool = True
